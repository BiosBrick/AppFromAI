...
function ProviderOption({
  ...
  value: 'ollama' | 'openai' | 'claude' | 'local';
  current: 'ollama' | 'openai' | 'claude' | 'local';
  label: string;
  hint: string;
  onSelect: () => void;
  last?: boolean;
...
/* ── Local Model row — navigates to model picker screen ── */
function LocalModelOption({
  ...
  const active = current === 'local';
  const displayHint = active && activeModelName ? activeModelName : hint;
  return (
    <Pressable style={[s.row, s.localRow]} onPress={onPress}>
      <View style={s.rowLabels}>
        <Text style={[s.rowLabel, active && s.rowLabelActive]}>Local Model</Text>
        <Text style={s.rowHint}>{displayHint}</Text>
      </View>
      <View style={s.localRowRight}>
        <View style={[s.radio, active && s.radioActive]}>
          {active ? <View style={s.radioDot} /> : null}
        </View>
        ...
        {!settings.useMock ? (
          <>
            <Section title={t.sectionProvider}>
              ...
              <LocalModelOption
                current={settings.provider}
                activeModelName={settings.localModelId || undefined}
                hint={t.providerLocalHint}
